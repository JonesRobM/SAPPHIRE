    ...
    for i in range(Start, End, Step):
        ...
        if i%Skip==0 and bool(globals()['pdf']) is True:
            ...
            try:
                if bool(System['Hetero']*globals()['HePDF']) is True:
                    result_cache['heteropos'] = DistFuncs.Hetero(result_cache['pos'], metadata['Species'], metadata['Elements'])
        ...
        try:
            if bool(System['Hetero']*globals()['HeAdj']) is True:
                result_cache['HeDist'] = DistFuncs.Hetero(result_cache['pos'], metadata['Species'], metadata['Elements'])
                if result_cache['heteropos'] is not None:
